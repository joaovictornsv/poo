...
HugeInteger &HugeInteger::operator=(const string &value) {
  this->input(value);
  return *this;
};

HugeInteger HugeInteger::operator+(HugeInteger &hugeInteger2) {
  HugeInteger hugeInteger3;

  hugeInteger3 = this->num;

  hugeInteger3.add(hugeInteger2);

  return hugeInteger3;
};

void HugeInteger::operator+=(HugeInteger &hugeInteger2) {
  this->add(hugeInteger2);
};

bool HugeInteger::operator==(HugeInteger &hugeInteger2) {
  return this->isEqualTo(hugeInteger2);
};

bool HugeInteger::operator!=(HugeInteger &hugeInteger2) {
  return this->isNotEqualTo(hugeInteger2);
};

bool HugeInteger::operator>(HugeInteger &hugeInteger2) {
  return this->isGreaterThan(hugeInteger2);
};

bool HugeInteger::operator>=(HugeInteger &hugeInteger2) {
  return this->isGreaterThanOrEqual(hugeInteger2);
};

bool HugeInteger::operator<(HugeInteger &hugeInteger2) {
  return this->isLessThan(hugeInteger2);
};

bool HugeInteger::operator<=(HugeInteger &hugeInteger2) {
  return this->isLessThanOrEqual(hugeInteger2);
};

ostream &operator<<(ostream &out, HugeInteger &hugeInteger) {
  for (int i = 0; i < hugeInteger.tamanho; i++) {
    out << hugeInteger.num[i];
  };
  out << endl;
